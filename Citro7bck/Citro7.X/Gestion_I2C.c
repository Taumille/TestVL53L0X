...
// 5 = nb in std_com
// 10 = data std_com
// 30 = QEI (à lire en 2 ou 4 octets)
// en wr :
// 10 = std_com
// 20 = PWM 2 octets

uint32_t I2C_Data_Shifter = 0;
...
void __attribute__((interrupt,auto_psv)) _SI2C1Interrupt (void){
    ...
    if (I2C1STATbits.RBF) {
        ...
        if (!I2C1STATbits.D_A) {  //si la dernière donnée reçut c'est une addresse
            ...
            I2C1CONbits.ACKDT = 0;
            I2C1CONbits.ACKEN = 1;
            if (State_I2C == 1) {   // si c'est le premier octet de data qu'on 
                Reg_Slave = Data;
                State_I2C ++;
            } else {
                if (Reg_Slave == 10) {
                    Buff_To_Std_In(&Data, 1);
                } else if (Reg_Slave == 20) {
                    I2C_Data_Shifter = Data;
                    Reg_Slave ++;
                } else if (Reg_Slave == 21) {
                    I2C_Data_Shifter = (I2C_Data_Shifter << 8) + Data;
                    Reg_Slave ++;
                    // must send I2C_Data_Shifter to PWM
                    int16_t vali16 = I2C_Data_Shifter & 0xFFFF;
                    Set_Motor_PWM(vali16/100.0);
                } else if (Reg_Slave == 31) {
					Val_Servo1 = Data;
                } else if (Reg_Slave == 32) {
					Val_Servo2 = Data;
				}
            }
        }
    ...
    if (!I2C1CONbits.SCLREL) {
        if (Read_Write_status && !I2C1STATbits.TBF) {
            ...
            if (Reg_Slave == 5) {
                uint16_t val16 = Std_Out_Data_Count();
                if (val16 > 255) {
                    Data_To_Send = 255;
                } else {
                    Data_To_Send = val16;
                }
            } else if (Reg_Slave == 10) {
                Get_Std_In(&Data_To_Send);
            } else if (Reg_Slave == 30) {
                // Get QEI
                I2C_Data_Shifter = 0x123456;
                Reg_Slave ++;
                Data_To_Send = I2C_Data_Shifter;
                I2C_Data_Shifter = I2C_Data_Shifter >> 8;
            } else if (Reg_Slave == 31) {
                Data_To_Send = I2C_Data_Shifter;
                I2C_Data_Shifter = I2C_Data_Shifter >> 8;
            }
